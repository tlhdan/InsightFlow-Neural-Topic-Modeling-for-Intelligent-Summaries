...
import sys
sys.path.append('..')
from my_utils import evaluate_topic_quality, smooth_curve
...
class BATM:
    ...
    def get_theta(self, bow):
        if self.device:
            bow = bow.to(self.device)  # Đưa dữ liệu về đúng device
        theta = self.encoder(bow)  # Gọi encoder để lấy theta
        return theta
